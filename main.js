...
(function() {
  var DEFAULT_WWW, VERSION, _, apache, apacheconf, exec, fs, getDirectories, i, inquirer, isRoot, items, len, nodegit, path, platform, questions, sys, wamp;
  ...
  getDirectories = function(srcpath) {
    return fs.readdirSync(srcpath).filter(file((function(_this) {
      return function() {
  ...
  if (_.includes(platform.os.toString().toLowerCase(), 'win')) {
    ...
    for (i = 0, len = items.length; i < len; i++) {
      apache = items[i];
      if (apache.lastIndexOf('apache', 0) === 0) {
